import argparse
from helpers import *
from scale_swarm import *
from docker_services import print_table, get_service_nodes, get_nodes_ps
...
def parse_args():
    ...
    parser.add_argument("--rebuild", "-r", action="store_true", help="rebuild the docker images")
    parser.add_argument("--scale", action="store_true", help="replicate microservices")
...
def setup_socialnetwork(nodes, ssh_commands, master_ip):
    cmd = "python3 scripts/init_social_graph.py --graph=socfb-Reed98"
    run_ssh_cmd(nodes[0], cmd, check=True)
...
def restart_stack(nodes, dsb_path, csv_file=None, compose="docker-compose-swarm.yml", env=".env", no_print=True, pin=False, 
                  cleanup_nodes=True, scale=True):
    ...
    if csv_file is not None:
        if scale:
            scale_services(nodes, csv_file)
        if pin:
            pin_cpus(nodes, csv_file)
    return table
...
def pin_cpus(nodes, csv_file, sort_cpus=False):
    ...
    with open(csv_file, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            if node_num == -1 or node_num >= len(nodes):
                print(f"node {node_num} not available")
                assert(False)
...
def set_compose_env(nodes, compose_file, csv_file, fraction=False):
    ...
    make_compose(csv_file, ".env", nodes, ",", frac_cores=fraction)
...
def main():
    ...
    if args.compose_file is None:
        compose_file = f"docker-compose-swarm-{current_service}.yml"
    else:
        compose_file = args.compose_file
    
    # rebuild docker images if flag set
    if args.rebuild:
    ...
    set_compose_env(nodes, compose_file, args.csv_file, fraction=args.fraction)
    ...
    restart_stack(nodes, DSB_PATH, csv_file=args.csv_file, compose="docker-compose-swarm.yml", env=".env", cleanup_nodes=True, no_print=True)

    # scale services if flag set
    if args.scale:
        print("scaling services...", flush=True)
        scale_services(nodes, args.csv_file)
