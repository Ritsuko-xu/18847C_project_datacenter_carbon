...
def parse_args():
    ...
    parser.add_argument("--output-raw", action="store_true", help="output raw wrk output")
    ...
    parser.add_argument("--compose-file", type=str, default=None, help="yaml file with service assignments")
    parser.add_argument("--node", type=int, default=0, help="node to run on")
    parser.add_argument("--jaeger", action="store_true", help="store jaeger traces")
    parser.add_argument("--jaeger-ip", type=str, default=None, help="ip address of jaeger backend")
    parser.add_argument("--partial", action="store_true", help="as you go, dump partial pickle file")
    ...
    group.add_argument("--load", type=int, default=None, help="single load to run")
...
def run_workload(nodes, wrk_type=0, input_rate=2000, time=30, warmup=0, 
    ...
    if warmup > 0:
        ...
        run_ssh_cmd(nodes[wrk_node], f"{CD_WRK} && sudo {warmup_cmd}", check=True)
    ...
    return run_ssh_cmd(nodes[wrk_node], f"{CD_WRK} && sudo {to_run}", check=True), to_run
...
def get_stats(nodes, docker=True, power=True, cpufreq=True, raw=False, prefix=""):
    node_stats = []
    node_rapls = []
    node_cpufreqs = []
    if docker:
        docker_stats = asyncio.run(run_remote_async(nodes, f"cat {prefix}stats.txt"))
    ...
    if power:
        rapl_stats = asyncio.run(run_remote_async(nodes, f"cat {prefix}rapl.txt"))
    ...
    if cpufreq:
        cpufreq_stats = asyncio.run(run_remote_async(nodes, f"cat {prefix}cpufreq.txt"))
...
def run_loads(nodes, loads, runtime=30, threads=2, sweep=False, cooldown=10, 
              ...
              restart=None, pin=False, wrk_node=0, jaeger=False, jaeger_ip=None,
              partial=None):
    ...
    out_stats = []
    idle_stats = None
    record = stats or power or cpufreq

    # loop through loads to run and get outputs
    for i, l in enumerate(loads):
        # perform this inner loop in a try - if an error occurs, sleep and try again
        max_tries = 5
        ...
        if restart is not None:
            ...
            restart_stack(nodes, DSB_PATH, csv_file=restart, pin=pin)

        if i == 0:
            if stats:
                asyncio.run(run_remote_async(nodes, f"./docker_stats.sh idle_stats.txt 5", background=False))
            if power:
                asyncio.run(run_remote_async(nodes, f"./rapl.sh idle_rapl.txt 5", background=False))
            if cpufreq:
                asyncio.run(run_remote_async(nodes, f"./cpufreq.sh idle_cpufreq.txt 5", background=False))

            time.sleep(6)
            
            if record:
                idle_stats = get_stats(nodes, docker=stats, power=power, cpufreq=cpufreq, prefix="idle_")
                idle_stats = [(0, idle_stats)]
        ...
        end_time = get_datetime(compact=False)
        print(f"load {l} done at {end_time}")
        ...
        if jaeger:
            workloads = get_current_service_workloads()
            workload_name = workloads[workload]["workload_type"]
            traces = get_traces(nodes, end_time=end_time, lookback=runtime, 
                                limit=1000, jaeger_ip=jaeger_ip, service_name=service_to_jaeger[workload_name])
            out.append(traces)
        raw_out = [l, cp]
        outs.append(out)
        if partial is not None:
            data_dump(outs, partial, overwrite=True)
    ...
    return outs, out_stats, idle_stats, raw_outs
...
def main():
    ...
    # run the load sweep
    sweep = args.loads is None and args.load is None
    ...
    if args.restart is not None:
        compose_file = args.compose_file if args.compose_file is not None else f"docker-compose-swarm-{service_name}.yml"
        set_compose_env(nodes, compose_file, args.restart, False)

    pickle_file_out = args.pickle_file if args.pickle_file is not None else f"load_sweep_{get_datetime(compact=True)}"
    if args.partial:
        partial = pickle_file_out
    else:
        partial = None

    data, stats, idle_stats, raw_data = run_loads(nodes, loads, runtime=args.time, threads=args.threads, sweep=sweep, 
                                                  ...
                                                  pin=args.pin, wrk_node=args.node, jaeger=args.jaeger, jaeger_ip=args.jaeger_ip,
                                                  partial=partial)
    ...
    if args.stats or args.power or args.cpufreq:      
        ...
        data_dump(idle_stats, "stats/" + args.pickle_file + "_IDLE", append=args.append)
